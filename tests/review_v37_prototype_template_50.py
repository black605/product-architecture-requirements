...
REQUIRED_GROUPS: dict[str, list[list[str]]] = {
    ...
    "asset": [["ASC", "素材", "Slot", "占位"], ["比例", "尺寸"], ["状态", "加载", "错误"], ["不继承", "不复用", "禁止"]],
...
JOURNEY_GROUPS: dict[str, list[list[str]]] = {
    ...
    "受控 Generation Request": [["Generation Request", "生成请求"], ["controlled", "受控"], ["不允许任意", "禁止"], ["PRS", "会话"], ["ready-to-generate", "draft", "草案"]],
    "没有原型能力": [["Generation Request", "生成请求"], ["没有", "无可用", "不可调用", "受能力限制"], ["不能声明", "不声称", "不能声称", "尚未生成"], ["ready-to-generate", "已准备", "Ready", "Blocked"]],
    ...
    "Patch 基线冲突": [["Patch", "PATCH"], ["v0.2"], ["v0.4"], ["冲突", "conflicted"], ["不做静默覆盖", "不静默覆盖", "不覆盖", "不应用"]],
    "L3 权限变化": [["L3"], ["权限", "可见"], ["DEC", "CHG"], ["不直接", "暂不能直接", "不能直接", "暂未应用", "暂不应用", "暂不修改原型"], ["Blocked", "待确认", "needs-decision"]],
    "技术通过但未登记": [["技术"], ["不能登记", "不得登记"], ["目标任务"], ["Owner"], ["候选"]],
    "证据齐备申请登记": [["TMF", "Manifest"], ["Owner"], ["scope", "适用范围"], ["授权"], ["尚未", "写入前"]],
    "登记返回审计": [["registered", "已登记"], ["Registry"], ["未注册", "尚未注册"], ["生产"], ["不能", "不得"]],
    "破坏性模板升级": [["主版本", "major"], ["重新验证"], ["旧版", "历史"], ["迁移"], ["P0", "主操作"]],
    "模板停用": [["deprecated", "停用"], ["新项目"], ["历史项目", "已有项目", "旧项目"], ["替代"], ["保留", "不删除"]],
    "只提供 Figma 标题": [["不可访问"], ["不能声称", "不得声称"], ["具体", "参数"], ["证据"]],
    "截图只能证明可观察内容": [["可观察"], ["不能", "不推断", "不从"], ["权限"], ["算法"], ["证据", "来源"]],
    "旧页面生产能力误判": [["可观察"], ["Mock"], ["真实能力"], ["生产证据"], ["不能", "不证明"]],
    "多来源冲突": [["冲突"], ["待确认"], ["不自行", "不能自行", "不选择", "不判定", "不做默认选择"], ["Profile"], ["证据", "来源"]],
...
def contract_check(record: dict[str, object]) -> tuple[bool, list[str]]:
    ...
    if focus == "session" and journey not in PATCH_EXPECTATIONS:
        ...
        if not contains_any(text, ["Generation Request", "生成请求", "预览"]):
