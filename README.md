...
产物是“需求与架构规格”，不是可直接上线的后端代码或 Figma 源文件；它可以为后续原型、视觉设计、接口设计和开发任务拆分提供明确输入。
...
可以。直接用自然语言修改即可，例如“把反馈渠道首期改为仅系统内消息”。Skill 应回写受影响的流程、功能和页面状态。

### 能否生成代码或完整视觉稿？

本 Skill 的职责是把需求与产品架构说明清楚。它可以输出组件规格、交互说明和交接清单；真正的代码实现或 Figma/Sketch 文件应交给相应的开发、原型或设计流程。
