...
4. Ready for Prototype 网关：高影响歧义进入 Needs Decision；通过后生成 v0.3/v0.4 Handoff Manifest。
5. 原型与验证节点：按需生成中保真线稿，并分开记录技术、目标用户和交接证据。
6. Handoff Ready 网关：核对 Must、Owner、依赖、风险和 DEC/F/P/TR/AC/T/ART 追溯。
7. 可选运营节点：研究、竞品、路线图、阶段汇报或指标复盘；结果写回 Snapshot，不另建 PRD。
8. 返回审计节点：核对范围、状态、Mock/真实边界和新增假设；未验证产物不得标记 Completed。
